...
conditions := [
  [
    ...
    {"condition": "member must not be public","attribute_path": ["member"],"values": ["allUsers","allAuthenticatedUsers"],"policy_type": "blacklist"}
  ],
  [
    ...
    {"condition": "member must not be domain-scoped","attribute_path": ["member"],"values": ["domain:example.com"],"policy_type": "blacklist"}
  ],
  [
    {"situation_description": "Only supported IAM member prefixes",
     "remedies": ["Use user:, group:, or serviceAccount: prefixes"]},
    {"condition": "member must use allowed prefix","attribute_path": ["member"],"values": ["users:jane@example.com"],"policy_type": "blacklist"}
  ],
  [
    {"situation_description": "Member strings must be clean",
     "remedies": ["Trim spaces; avoid whitespace"]},
    {"condition": "member must not contain whitespace","attribute_path": ["member"],"values": [" user:jane@example.com "],"policy_type": "blacklist"}
  ],
  [
    ...
    {"condition": "member must be a corporate email","attribute_path": ["member"],"values": ["user:jane@gmail.com"],"policy_type": "blacklist"}
  ],
  [
    {"situation_description": "User/Group emails must be syntactically valid",
     "remedies": ["Fix malformed email (double '@')"]},
    {"condition": "member must not contain malformed email","attribute_path": ["member"],"values": ["group:eng@@example.com"],"policy_type": "blacklist"}
  ],
  [
    ...
    {"condition": "serviceAccount email format must be valid","attribute_path": ["member"],"values": ["serviceAccount:not-an-email"],"policy_type": "blacklist"}
  ]
]
