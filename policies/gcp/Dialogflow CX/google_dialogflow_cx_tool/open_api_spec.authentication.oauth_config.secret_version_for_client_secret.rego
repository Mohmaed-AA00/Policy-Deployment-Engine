package terraform.gcp.security.dialogflow_cx.google_dialogflow_cx_tool.open_api_spec_authentication_oauth_config_secret_version_for_client_secret
...
secret_version_path := ["open_api_spec", 0, "authentication", 0, "oauth_config", 0, "secret_version_for_client_secret"]
...
message := [
  ...
  "Situation 1: Dialogflow CX Tool OAuth client secrets must use a Secret Manager secret version.",
  sprintf("Non-Compliant Resources: %s", [non_compliant_display]),
  "Potential Remedies: Set secret_version_for_client_secret to projects/{project}/secrets/{secret}/versions/{version}."
]

details := [{
  "situation": "Dialogflow CX Tool OAuth client secrets must use a Secret Manager secret version.",
  "remedies": ["Set secret_version_for_client_secret to projects/{project}/secrets/{secret}/versions/{version}."],
