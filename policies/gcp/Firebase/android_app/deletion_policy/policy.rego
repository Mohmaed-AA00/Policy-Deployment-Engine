...
conditions := [
    [
    {"situation_description" : "The Firebase android_app resource 'testfirebaseandroidapp-nc' is configured with 'deletion_policy' set to 'ABANDON'. This may leave underlying configurations or metadata undeleted, leading to compliance risks, residual data exposure, or resource mismanagement. To enforce proper lifecycle management, the deletion_policy must be set to 'DELETE'",
    "remedies":[  "Update the deletion_policy for the Firebase android_app resource to 'DELETE' instead of 'ABANDON'.",
    ...
    {
        "condition": "Firebase android_app 'testfirebaseandroidapp-nc' has 'deletion_policy' set to 'ABANDON'. It should be set to '[\"DELETE\"]' to ensure that resources are fully removed when deleted.",
