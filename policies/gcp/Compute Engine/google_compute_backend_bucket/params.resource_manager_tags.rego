package terraform.gcp.security.compute_engine.google_compute_backend_bucket.params_resource_manager_tags
...
conditions := [
	[
		{
			"situation_description": "The Backend Bucket contains missing or structurally invalid Resource Manager tags.",
			"remedies": [
...
violations := [
{
	"name": resource_name,
	"message": sprintf(
		"Backend Bucket '%s' must use numeric Resource Manager tag references in the form tagKeys/{id} = tagValues/{id}.",
		[resource_name],
	),
} |
...
situation_results := [
	{
		"situation": "The Backend Bucket contains missing or structurally invalid Resource Manager tag references.",
		"remedies": [
