...
conditions := [
    [
        {
            "situation_description": "Compute disk uses an RSA-wrapped customer-supplied encryption key (CSEK), which embeds key material in configuration.",
            "remedies": ["Remove disk_encryption_key.rsa_encrypted_key and use disk_encryption_key.kms_key_self_link (CMEK) instead."]
        },
        {
            "condition": "disk_encryption_key.rsa_encrypted_key should not be used.",
