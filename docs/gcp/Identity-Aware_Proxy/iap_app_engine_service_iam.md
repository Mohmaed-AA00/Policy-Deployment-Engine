...
|----------|-------------|----------|-----------------|-----------|-----------|---------------|
| `app_id` | App Engine application ID used in the IAP resource path (typically the project ID). Identifies which App Engine app’s service IAM is being managed. | true | true | Must point to the correct application; mis-scoping could attach IAM to the wrong app. | app_id = "my-gcp-project" | app_id = "" |
| `service` | App Engine service name (for example, "default" or a custom service) whose IAP IAM is managed. | true | true | Scopes the IAM binding to the intended service; a wrong name could leave the target unprotected. | service = "default" | service = "nonexistent" |
...
| `role` | `google_iap_app_engine_service_iam_binding`/`..._member` grant a specific role. Custom roles must be of the form `[projects|organizations]/{parent}/roles/{role-name}`. | true | true | Use least privilege. The typical accessor is roles/iap.httpsResourceAccessor. | role = "roles/iap.httpsResourceAccessor" | role = "roles/owner" or another broad/unrelated role |
| `policy_data` | A `google_iam_policy` data source (authoritative when used with the *_policy resource). | true | true | Authoritative replacement of IAM; incorrect data can remove required bindings. | policy_data references data.google_iam_policy with intended bindings (for *_policy). | policy_data missing required bindings or mis-scoped to the wrong resource. |
