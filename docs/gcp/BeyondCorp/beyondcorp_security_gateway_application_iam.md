## 🛡️ Policy Deployment Engine: `beyondcorp_security_gateway_application_iam`

This section provides a concise policy evaluation for the `beyondcorp_security_gateway_application_iam` resource in GCP.

Reference: [Terraform Registry – beyondcorp_security_gateway_application_iam](https://registry.terraform.io/providers/hashicorp/google/latest/docs/resources/beyondcorp_security_gateway_application_iam)
...
| `application_id` | The application identifier. The name is formatted as follows: `projects/{project_id}/locations/{location_id}/applications/{application_id}`. | true | false | The application ID is required to identify the specific BeyondCorp Application for which the IAM policy is being managed. | None | None |
...
| `role` | `google_beyondcorp_security_gateway_application_iam_binding` can be used per role. Note that custom roles must be of the format `[projects|organizations]/{parent-name}/roles/{role-name}`. | true | false | The role field is necessary to specify the permissions being granted to the members. | None | None |
| `policy_data` | The data resource policy data. See below for structure. | true | false | The policy data is required to define the IAM policy being applied to the BeyondCorp Application. | None | None |
...
| `title` | The description of the expression. This is a longer text which describes the expression, e.g. when hovered over it in a UI. | true | false | The title of the condition is necessary for identifying the condition in various contexts. | None | None |
